...
Zhushen Hub 使用**简单的控制台日志**（不依赖日志库）。日志写入 stdout/stderr，由进程管理器或容器运行时捕获。
...
```typescript
console.log('Zhushen Hub 正在启动...')
console.log('[Hub] ZS_LISTEN_PORT: 3000（environment）')
console.log('[Web] Hub 正在监听 :3000')
console.log('Zhushen Hub 已就绪！')
```
