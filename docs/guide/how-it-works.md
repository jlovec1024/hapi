# 工作原理（How it Works）

主神由三个相互协作的组件组成，用于实现 AI Agent 的远程控制。

## 架构总览

```
┌────────────────────────────────────────────────────────────────────────────┐
│                     Your Machine (Local or Hub Host)                       │
│                                                                            │
│   ┌──────────────┐         ┌──────────────┐         ┌──────────────┐       │
│   │              │         │              │         │              │       │
│   │  Zhushen CLI │◄───────►│ Zhushen Hub  │◄───────►│   Web App    │       │
│   │              │ Socket  │              │   SSE   │  (embedded)  │       │
│   │  + AI Agent  │   .IO   │  + SQLite    │         │              │       │
│   │              │         │  + REST API  │         │              │       │
│   └──────────────┘         └──────┬───────┘         └──────────────┘       │
│                                   │                                        │
│                                   │ localhost:3006                         │
└───────────────────────────────────┼────────────────────────────────────────┘
                                    │
                            浏览器 / 手机 / PWA
                                    │
┌───────────────────────────────────┼────────────────────────────────────────┐
│                           访问设备与网络                                   │
│                                   │                                        │
│         ┌─────────────────────────┼─────────────────────────┐              │
│         │                         ▼                         │              │
│         │    ┌──────────────┐           ┌──────────────┐    │              │
│         │    │              │           │              │    │              │
│         │    │    PWA /     │           │   Browser    │    │              │
│         │    │   Phone      │           │              │    │              │
│         │    │              │           │              │    │              │
│         │    └──────────────┘           └──────────────┘    │              │
│         │                                                   │              │
│         └───────────────────────────────────────────────────┘              │
│                            Your Phone                                      │
└────────────────────────────────────────────────────────────────────────────┘
```

> **说明**：Hub 可以运行在本地桌面，也可以运行在远程主机（VPS、云主机等）。你可以通过局域网、HTTPS 或其他受控网络方式访问它。

## 组件说明

### 主神 CLI

CLI 是对 AI 编程代理（Claude Code、Codex、Cursor Agent、Gemini、OpenCode）的封装，它负责：

- 启动并管理编码会话
- 将会话注册到主神 Hub
- 转发消息与权限请求
- 提供 MCP（Model Context Protocol）工具

**关键命令：**
```bash
zs               # 启动 Claude Code 会话
zs codex         # 启动 OpenAI Codex 会话
zs gemini        # 启动 Google Gemini 会话
zs opencode      # 启动 OpenCode 会话
zs runner start  # 启动后台服务，用于远程拉起会话
```

### 主神 Hub

Hub 是连接全系统的中枢服务：

- **HTTP API**：用于会话、消息、权限等 REST 接口
- **Socket.IO**：与 CLI 之间的实时双向通信
- **SSE (Server-Sent Events)**：向 Web 客户端推送实时更新
- **SQLite Database**：持久化存储会话和消息

### Web App

基于 React 的 PWA，提供移动端控制界面：

- **Session List**：查看所有活跃/历史会话
- **Chat Interface**：发送消息、查看 Agent 回复
- **Permission Management**：批准或拒绝工具权限
- **File Browser**：浏览项目文件并查看 git diff
- **Remote Spawn**：在任意已连接机器上启动新会话

## 数据流

### 启动会话流程

```
1. 用户在终端运行 `zs`
         │
         ▼
2. CLI 启动 Claude Code（或其他 Agent）
         │
         ▼
3. CLI 通过 Socket.IO 连接 Hub
         │
         ▼
4. Hub 在数据库创建会话
         │
         ▼
5. Web 客户端通过 SSE 收到更新
         │
         ▼
6. 会话出现在移动端应用中
```

### 权限请求流程

```
1. AI Agent 请求工具权限（例如编辑文件）
         │
         ▼
2. CLI 将权限请求发送到 Hub
         │
         ▼
3. Hub 存储请求并通过 SSE 通知
         │
         ▼
4. 用户在手机收到通知
         │
         ▼
5. 用户在 Web App 中批准/拒绝
         │
         ▼
6. Hub 通过 Socket.IO 将决策回传 CLI
         │
         ▼
7. CLI 通知 AI Agent，执行继续
```

### 消息流

```
User (Phone)                 Hub                     CLI
     │                         │                       │
     │──── Send message ──────►│                       │
     │                         │─── Socket.IO emit ───►│
     │                         │                       │
     │                         │                       ├── AI processes
     │                         │                       │
     │                         │◄── Stream response ───│
     │◄─────── SSE ────────────│                       │
     │                         │                       │
```

## 通信协议

### CLI ↔ Hub：Socket.IO

用于实时双向通信：
- 会话注册与心跳
- 消息转发（用户输入 → Agent）
- 权限请求与响应
- 元数据与状态更新
- RPC 方法调用

### Hub ↔ Web：REST + SSE

- **REST API**：执行动作（发送消息、审批权限）
- **SSE stream**：接收实时更新（新消息、状态变化）

### 外部访问

主神本身负责会话、状态同步与远程控制；访问方式由你的部署环境决定，例如：
- **局域网访问**：同一网络中的浏览器或手机直接访问 Hub
- **HTTPS 访问**：通过你自己的域名和反向代理提供访问入口
- **远程主机访问**：将 Hub 部署在可访问的自有主机上

## 无缝接管（Seamless Handoff）

主神的核心能力是：在本地终端与远程设备之间切换控制权时，不丢失会话状态。

### 本地模式（Local Mode）

本地模式下是完整终端体验（原生 Claude Code / Codex / OpenCode）：

- 键盘直接输入，响应即时
- 完整终端 UI（含高亮等）
- 适合专注、连续编码
- AI 处理运行在本机

### 远程模式（Remote Mode）

需要离开工位时可切换远程模式：

- 通过 Web/PWA 在任意设备控制
- 随时审批权限请求
- 离开桌面时持续监控进度
- 会话仍在你的本机继续运行
- 终端页面重新接管时会主动清理旧连接，减少重复连接和重连异常

### 切换机制

```
┌─────────────────┐                    ┌─────────────────┐
│   Local Mode    │◄──────────────────►│   Remote Mode   │
│   (Terminal)    │                    │   (Phone/Web)   │
└─────────────────┘                    └─────────────────┘
        │                                      │
        │  ┌────────────────────────────┐      │
        └─►│  Same Session, Same State  │◄─────┘
           └────────────────────────────┘
```

**Local → Remote：**
- 手机/Web 发来消息
- 会话自动切换为远程模式
- 终端显示 “Remote mode - waiting for input”

**Remote → Local：**
- 在终端按两次空格
- 立即夺回本地控制
- 可无缝继续输入

### 典型场景

1. **离岗远程控制**：在桌面启动会话，通勤或短暂离开时通过手机继续操作。

2. **远程审批权限**：AI 请求文件权限，手机收到通知，一键批准后继续执行。

3. **多设备协同**：桌面负责重任务处理，手机负责查看进度与轻操作。
