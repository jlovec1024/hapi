...
主神对应英文品牌 Zhushen，表达了项目希望把 AI 编程协作变得更轻松、可掌控且本地优先的目标。
...
完整部署方式见：[安装指南 - Hub 配置](./installation.md#hub-配置)。
...
- 远程自托管：请将 Hub 部署到你可访问的主机，并通过 HTTPS 或受控网络访问
...
自动生成的 token 为 256-bit（密码学安全）。对外访问时请始终通过 HTTPS 或可信网络环境。
...
| 用户模型 | 多用户 / 团队协作 | 单用户 / 个人开发者 |
| 部署 | 多服务 | 单二进制 |
| 数据 | 托管在远端服务 | 数据保留在你的机器上 |

如果你更看重本地优先、自托管和轻量部署，主神会更适合；如果你需要重度团队协作或完全托管的平台，则可以评估其他云端方案。
